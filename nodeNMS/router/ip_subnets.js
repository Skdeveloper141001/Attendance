...
router.get('/',(req,res)=>{
    var query=`SELECT l.name as location ,ips.* FROM ip_subnets as ips INNER JOIN locations as l ON l.id=ips.location_id  where ips.delete_flag='N'`;
...
router.get('/:id',(req,res)=>{
    ...
    var query="SELECT * from ip_subnets WHERE id=?";
    var id=req.params.id;
    ...
    mysql.query(query,[id]).then((result)=>{
        ...
        return res.status(404).send("IP_subnet Not Found");
    });
});
...
router.post('/', (req, res) => {
    ...
    const query = "INSERT INTO  ip_subnets SET ?";
...
router.put('/:id',(req,res)=>{
    ...
    var id = req.params.id;
    var values = req.body;
    var query = "UPDATE ip_subnets SET ? WHERE id = ? ";
...
router.delete('/:id',(req,res)=>{
    ...
    var query = "UPDATE ip_subnets SET delete_flag='Y' WHERE id = ? ";
