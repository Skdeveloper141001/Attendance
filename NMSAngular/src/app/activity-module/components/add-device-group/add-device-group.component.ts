...
@Component({
  ...
  styleUrl: './add-device-group.component.scss'
})
export class AddDeviceGroupComponent  implements OnInit{
  ...
  constructor( private apiService: DeviceinfoService ,private datepipe:DatePipe,private http: HttpClient,@Optional() private dialogRef: MatDialogRef<AddDeviceGroupComponent>, @Inject(MAT_DIALOG_DATA) public data: any
) { }
...
onSubmit() {
  ...
  const formValue = this.deviceform.value;
   ...
   this.apiService.postData('dGroup', payload).subscribe(response => {
...
onUpdate(id:any): void {
  ...
  this.apiService.putData('dGroup/'+id, payload).subscribe({
    next: (response) => {
