...
import { AddIPSubnetComponent } from '../add-ipsubnet/add-ipsubnet.component';
...
@Component({
  selector: 'app-ipsubnet',
  templateUrl: './ipsubnet.component.html',
  styleUrl: './ipsubnet.component.scss'
})
export class IPSubnetComponent {
  ...
  displayedColumns: string[] = [
    'name','cidr_block','gateway','vlan_id','location','description','create_date','actions'
    ];
  ...
  openAddDeviceWindow() {
    const dialogRef = this.dialog.open(AddIPSubnetComponent , {
  ...
  getRoom() {
    this.apiService.getData('ipsubnets').subscribe(response => {
  ...
  this.apiService.deleteData('ipsubnets/'+id).subscribe(response => {
